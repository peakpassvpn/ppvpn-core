...
| `selection` | object | 是 | v1 仅支持 `mode=manual`，默认节点必须存在 |
| `routing.bypass_private` | boolean | 否 | 为 true 时私网目标直连 |
...
| `transport` | v1 只能缺省或 `type` 为空；非空传输会被拒绝 |
