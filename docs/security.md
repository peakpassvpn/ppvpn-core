...
设备系统代理是另一个用途明确的端点：它跟随当前选择节点，只允许显式 loopback 地址，但为了操作系统代理兼容性当前不带认证。本机同一用户下的其他进程可能使用它。宿主仅应在用户明确开启系统代理模式期间启用，退出、崩溃恢复和切换模式时必须撤销系统代理设置；高隔离场景优先使用 TUN 或每节点认证代理。

## Profile 防护

- JSON 严格解码，未知字段、歧义 credential union 和尾随值全部失败关闭。
- 只接受固定协议集合；v1 拒绝未知 transport。
